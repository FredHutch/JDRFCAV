...
library(JDRFCAV)
# devtools::install_github("FredHutch/JDRFCAV", auth_token = "d91ceb23e7163ee5a7bd5cf748cf0de2e14cac83")
...
# Featureless_simulation<-resample(lrn_featureless, task_j, resampling = makeResampleDesc("Subsample", iter = 20, predict = 'both', split = 0.8), measures = list(rmse), models = TRUE)
# Featureless_simulation$measures.test$rmse %>% hist
...
# load( file = 'data/bmr_tib_Bench_CV_non_stratified.rdata') # bmr_tib

##################################################################
# out<-bmr_tib$Resample[[1]]$models[[1]]
# out$learner.model$next.model$learner.model %>% coef %>% tidy %>% nrow
# bmr_tib$Resample[[1]]$extract[[1]]$learner.model %>% coef %>% tidy %>% nrow

## Add n_coef from extract
bmr_tib %<>% 
  mutate(n_coef_from_extract = Resample %>% map('extract'))
# bmr_tib %<>% mutate(n_coef_from_model = Resample %>% map_int(~.x$model$learner.model$next.model$learner.model %>% coef %>% tidy %>% nrow))
# bmr_tib$n_coef_from_extract
...
# plot(bmr_tib$n_coef_from_extract %>% unlist, bmr_tib$rmse_h_n_vec %>% unlist); abline(h = rmse_null, col='red')



  ## Diagnostics RMSE distributions > rmse_null
  # bmr_tib %<>%  mutate(rmse_h_n_vec_vs_REF = rmse_h_n_vec %>% map(~(.x >rmse_null) %>% table) ) 
    #.x = bmr_tib$rmse_h_n_vec[[1]]
   # bmr_tib$rmse_h_n_vec_vs_REF
   # bmr_tib$rmse_h_n_vec %>% bind_cols %>% boxplot; abline(h=rmse_null, col='red')
```
...
#  labs(title = 'Benchmarking + resampling: Average RMSE for different pre-processing feature selection criteria', subtitle = 's=min, n=100, train/test = 80%, Univariate: top_n, Clustering: 1-h')
...
### Stratified resampling within median(y)
```{r eval=FALSE}
...
# C. proof of concept: single parameter value {h=0.3, top_n = 30}

# C1. sinlge split (holdout)
...
# C2. (Multiple) resample / Cross-validation
```{r eval = FALSE}
