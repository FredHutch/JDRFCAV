...
Active.Box.folder<-'c:/Users/dberel/Box Sync/'


# Design.file.box<-c('CAV Data Sharing/Cohort 2 Clinical and Demographic Data/C-peptide decline calculations - Skinner/data/CAV-Cohort-2_outcomes_parameters_20170323a.csv') ## old.
Design.file.box<-c('CAV Data Sharing/Cohort 2 Clinical and Demographic Data/C-peptide decline calculations - Skinner/data/CAV-Cohort-2_outcomes_parameters_20170727a.csv') ## August 1, Sam updated the outcomes/clinical file to include some missing values and corrected units for the c-peptide outcomes
...
FACS.design.file.box<-("CAV Data Sharing/FACS Immunophenotyping - Long/Cohort 2/FACS_designFile_baseline.csv")
...
# FACS.raw.file.box<-("CAV Data Sharing/FACS Immunophenotyping - Long/Cohort 2/FACS_data_baseline.csv") # old feature names
FACS.raw.file.box<-("CAV Data Sharing/FACS Immunophenotyping - Long/Cohort 2/CAVII_shorten_v2.csv")
...
# which(is.na(FACS.join), arr.ind=TRUE)[,'col']
# FACS.join[is.na(FACS.join)]<-0 ### impute with analyte(column) mean
...
mRNA.data_filt.T$barcode<-substr(rownames(mRNA.data_filt.T),2, 100)
# dim(mRNA.data_filt.T) # this include both baseline, AND day 1. will be filtered by the next inner_join

### Join (design + raw(filt))
mRNA.join<-inner_join(x=mRNA.design, y=mRNA.data_filt.T, by='barcode')
mRNA.join<-mRNA.join %>% dplyr::rename(participant_id=ITN_ID)
rownames(mRNA.join)<-mRNA.join$participant_id
mRNA.join<-mRNA.join[,19:21186]




###  3. Pro.Insulin
Pro.Insulin.all.file.box<-c("CAV Data Sharing/Proinsulin C Peptide Ratio - Gorus/Cohort 2/PIC_baseline.csv")
...
Pro.Insulin.all<-Pro.Insulin.all[,13:14]
...
### 4. microRNA
# Thoug did not end up used in down-selection analysis
microRNA.NEW.Design.file.box<-c("CAV Data Sharing/miRNA by Exiqon - Pugliese/Cohort 2/designFile_baseline.csv")
microRNA.NEW.Design<-read.csv(paste(Active.Box.folder, microRNA.NEW.Design.file.box, sep=''))[,1:9]

microRNA.NEW.norm.file.box<-c("CAV Data Sharing/miRNA by Exiqon - Pugliese/Cohort 2/normmiRNA_baseline.csv")
...
microRNA.NEW.join<-microRNA.NEW.join[,10:169]
...
# Bcells
RNAseq.Bcell.file.box<-"CAV Data Sharing/Presentations and Analysis - Skinner/Data processing and filtering for Dror/CAV-Cohort-2-CellSubset_data/CAV-Cohort-2-RNAseq_Bcell_data_20170224a.RData"

# CD4 T cells
RNAseq.CD4.file.box<-"CAV Data Sharing/Presentations and Analysis - Skinner/Data processing and filtering for Dror/CAV-Cohort-2-CellSubset_data/CAV-Cohort-2-RNAseq_CD4_data_20170224a.RData"

# CD8 T celle
RNAseq.CD8.file.box<-"CAV Data Sharing/Presentations and Analysis - Skinner/Data processing and filtering for Dror/CAV-Cohort-2-CellSubset_data/CAV-Cohort-2-RNAseq_CD8_data_20170224a.RData"

# Monocytes
RNAseq.Monocytes.file.box<-"CAV Data Sharing/Presentations and Analysis - Skinner/Data processing and filtering for Dror/CAV-Cohort-2-CellSubset_data/CAV-Cohort-2-RNAseq_monocyte_data_20170224a.RData"

# Wholeblood
RNAseq.Wholeblood.file.box<-"CAV Data Sharing/Presentations and Analysis - Skinner/Data processing and filtering for Dror/CAV-Cohort-2-WholeBlood_data/CAV-Cohort-2-RNAseq-WholeBlood_data_20170516b.RData" # 5/16/17
...
RNA.seq.pre.process.function<-function(i, Cell.type.input.Matrix){
  ...
  cur.Genes<-cur.ENSG[,-c(1,2,4,5)] %>%
    group_by(symbol) %>%
    summarise_each(funs(sum)) %>% ### !!! Sum, rather than mean
    #filter(!is.na(symbol)) %>%
    data.frame %>%
...
###### 6. Treg
Treg.all.file.box<-c("CAV Data Sharing/Presentations and Analysis - Skinner/Data processing and filtering for Dror/CAV-Cohort-2-Treg_Nanostring_signature/CAV-Cohort-2-Treg_data_20170425a")
...
# Bcells
RNAseq.Mod.Bcell.file.box<-"CAV Data Sharing/Presentations and Analysis - Skinner/Data processing and filtering for Dror/CAV-Cohort-2-CellSubset_data/CAV-Cohort-2-RNAseq_Bcell_WGCNAModules_20170327a.RData"

# CD4 T cells
RNAseq.Mod.CD4.file.box<-"CAV Data Sharing/Presentations and Analysis - Skinner/Data processing and filtering for Dror/CAV-Cohort-2-CellSubset_data/CAV-Cohort-2-RNAseq_CD4_WGCNAModules_20170327a.RData"

# CD8 T celle
RNAseq.Mod.CD8.file.box<-"CAV Data Sharing/Presentations and Analysis - Skinner/Data processing and filtering for Dror/CAV-Cohort-2-CellSubset_data/CAV-Cohort-2-RNAseq_CD8_WGCNAModules_20170327a.RData"

# Monocytes
RNAseq.Mod.Monocytes.file.box<-"CAV Data Sharing/Presentations and Analysis - Skinner/Data processing and filtering for Dror/CAV-Cohort-2-CellSubset_data/CAV-Cohort-2-RNAseq_monocyte_WGCNAModules_20170327a.RData"

# Wholeblood
RNAseq.Mod.Wholeblood.file.box<-"CAV Data Sharing/Presentations and Analysis - Skinner/Data processing and filtering for Dror/CAV-Cohort-2-WholeBlood_data/CAV-Cohort-2-RNAseq_WholeBlood_WGCNAModules_20170516b.RData"  # 5/16/17
...
## 8.1 eigen values
# B.Affy.eigen.file.box<-"CAV Data Sharing/Affy arrays from serum exposure expt - Hessner/Cohort 2/WGCNA/Affy_WGCNAeigenGenes.csv"
B.Affy.eigen.file.box<-"CAV Data Sharing/Affy arrays from serum exposure expt - Hessner/Cohort 2/WGCNA/Affy_WGCNAeigenGenes_noMod0.csv"  # revised, 8/11
...
B.Affy.module.set.file.box<-"CAV Data Sharing/Affy arrays from serum exposure expt - Hessner/Cohort 2/WGCNA/moduleAssignment.csv"
...
load('data/GSEA_gene_IDs_list_all.RData') # GSEA_gene_IDs_list_all %>% map(length)
...
metadata.raw.table<-tibble(Assay=names(MultiAssay.cohort.2),
                           Assay.type=rep('Long', 16)) ### rund Long for all assays, to be comparible to mlr PreProc
...
task<-makeRegrTask(data=mlr.Data, target=colnames(Y.raw)) # with covariates!
task
# data_task<-task # over-ride for testing purposes
# task %>% getTaskFeatureNames %>% substr(1, 8) %>% table()
