...
#(TO DO: reformat into Bioc::GSEABase GeneSetCollection format)
...
lrn_1<-Func_update_args_univ_clusters(lrn = lrn_PreProcess_glmnet, args_vec = args_1, lrn.id = 'lrn1')        # glmnet. NOT CV-glmnet!
# lrn_1$par.vals
```
...
```{r baked}
data_to_bake  <-task_j %>% getTaskData()
target_to_bake<-task_j %>% getTaskTargetNames()
...
```{r ML_from_train}
# set.seed(1)
Model_UnivClust<-train(lrn_1, task_j) ## lrn will have defaults s, that is used to extract/predict specific coef, though all s values are used for calculation
...
# model_intercept_only<-train(makeLearner(cl='regr.featureless'), task_j)
...
bmr_s_tib_cv<-data.frame(param = c(1:20)) %>% as_data_frame %>% 
