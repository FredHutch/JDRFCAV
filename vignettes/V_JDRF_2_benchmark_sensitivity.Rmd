...
## B.1: naive, nanual loop (via appply-like / map), broken to pre-processed/baked -> s.control (cv.glmnet min) -> ML
```{r warning = FALSE}
#############################################################################
# bmr_tib_naive<-bmr_tib %>% mutate(whole_run = lrn_i %>% imap(~whole_run_function(task = task_j, lrn = .x, lambda_opt = 'pushed_lambda', .y)))
# save(bmr_tib_naive, file = 'bmr_tib_Bench_only.rdata')
#############################################################################
load(file = 'C:/Users/dberel/Desktop/Bioc2mlr/bmr_tib_Bench_only.rdata') # bmr_tib_naive
...
DF_both2 %>% 
  ...
  filter(hclust_cutree_h == param.corrplot.n.clusters.h, Univ_top_n == param.Univ.filt.top.n.features) 
...
scale_y_continuous(breaks = pretty(DF_both2$rmse, n = 10)) +
...
# REF: bmr_all_comb
GS_comb_primary_tib<-data.frame(Primary_analyte = GS_comb$primary_analytes, stringsAsFactors = FALSE) %>% as_data_frame
GS_comb_primary_tib %<>% 
  mutate(bmr_all_comb_overlap_matrix_YN = Primary_analyte %>% map(~{
    # .x = GS_comb_primary_tib$Primary_analyte[[1]]
    # .x %in% bmr_all_comb$primary_analytes[[40]]
  
    bmr_all_comb %<>% 
    ## Primary
      mutate(primary_overlap_lgl_vec = primary_analytes %>% map_lgl(function(primary_comb_i) .x %in% primary_comb_i) %>% setNames(bmr_all_comb %>% unite('h_topN', c('hclust_cutree_h','Univ_top_n')) %>% pull(h_topN) )) %>% 
    
    ## Secondary
      mutate(secondary_overlap_lgl_vec = secondary_analytes %>% map_lgl(function(secondary_comb_i) .x %in% secondary_comb_i) %>% setNames(bmr_all_comb %>% unite('h_topN', c('hclust_cutree_h','Univ_top_n')) %>% pull(h_topN) )) %>% 
   
    
    ## Primary + Secondary (primary over secondary)
      mutate(PriSec__overlap_lgl_vec = map2_lgl(primary_overlap_lgl_vec, secondary_overlap_lgl_vec, any))
    
 }))
GS_comb_primary_tib$bmr_all_comb_overlap_matrix_YN[[1]] %>% 
  select(primary_overlap_lgl_vec, secondary_overlap_lgl_vec, PriSec__overlap_lgl_vec)
...
GS_comb_primary_tib %<>% 
  mutate(overlap_matrix_ggplot = map2(bmr_all_comb_overlap_matrix_YN, Primary_analyte, ~
    # .x = GS_comb_primary_tib$bmr_all_comb_overlap_matrix_YN[[1]]
    .x %>% 
      #select(hclust_cutree_h, Univ_top_n, PriSec__overlap_lgl_vec) %>% 
 ggplot() + geom_tile(aes(x = Univ_top_n, y = hclust_cutree_h, fill = PriSec__overlap_lgl_vec)) +
...
# GS_comb_primary_tib$overlap_matrix_ggplot[[1]]
