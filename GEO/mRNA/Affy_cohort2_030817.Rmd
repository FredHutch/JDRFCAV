...
Folder_location<-'GEO/mRNA/'

# read in data
expData<-read.csv(file = paste0(Folder_location, "CAV_IIandIII_All126wNormPla_PartekRMA_rawLog2Int_2016Dec09_forBRI.csv"), row.names=1)
dim(expData)
...
hessDesign<-read.csv(file=paste0(Folder_location, "20161122 Hessner ITN samples-ALL.csv"))
...
outcomeData<-read.csv(file=paste0(Folder_location, "CAV-Cohort-2_outcomes_parameters_20170221a.csv"))
...
# Marty's 2 lists
geneLists<-read.csv(file=paste0(Folder_location,"1374 and 359 lists.csv"))
...
affyData1<-read.csv(file=paste0(Folder_location, "Set65_CAVwROHC_PartekRMA_rawLog2Int_2016Feb12_wN35Anno.csv"))
...
#################################################################################################
#affyEsetFilter<-nsFilter(affyEset, remove.dupEntrez=FALSE, var.func=function(x) stats::IQR(x, type=1), var.cutoff=0.5)$eset
...
dim(affyEsetNoDateFilter)
#write.csv(affyEsetNoDate, file="/Users/ewhalen/Projects/CAV/Results/Cohort2/Affy/fullAffymetrixData_RemovedDateSent.csv")
write.csv(affyEsetNoDateFilter, file=paste0(Folder_location,"filteredAffymetrixData_RemovedDateSent.csv"))
...
write.csv(hessDesignSub0r, file=paste0(Folder_location,"designAtBaseline.csv"), row.names=FALSE)
